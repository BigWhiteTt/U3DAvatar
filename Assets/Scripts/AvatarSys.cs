...
public class AvatarSys : MonoBehaviour {
    ...
    public Transform girlSourceTrans;//资源model
    public GameObject girlTarget; //骨架物体，换装的人 
    
    public Dictionary<string, Dictionary<string, SkinnedMeshRenderer>> girlData = new Dictionary<string, Dictionary<string, SkinnedMeshRenderer>>();
    //小女孩所有的资源信息   //部位的名字，部位编号，部位对应的skm
    public Transform[] girlHips; //小女孩骨骼信息
    public Dictionary<string, SkinnedMeshRenderer> girlSmr = new Dictionary<string, SkinnedMeshRenderer>();// 换装骨骼身上的skm信息
    //部位的名字，部位对应的skm
    public string[,] girlStr = new string[,] { {"eyes","1"},{"hair","1"},{"top","1"},{"pants","1"},{"shoes","1"},{"face","1"}};
    //初始化信息
